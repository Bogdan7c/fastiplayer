...
## S01P intent-based read boundary (2026-07-20)
...
- `PlaylistQueue::items()` и ambiguous `len()` временно остаются documented S01Q bridges без `#[deprecated]`. Focused source inventory фиксирует ровно 47 `items()` и 31 ambiguous `len()` app-egui callsite и запрещает новые/возвращённые legacy callsites в migrated core/state.
- Verification: 82 playlist-core tests PASS on Rust 1.96; strict focused Clippy, workspace Rust 1.96 check, focused MSRV 1.92 check, rustfmt, Serena diagnostics and refactor guardrails PASS. Dependency audit unchanged: only known quick-xml RUSTSEC-2026-0194/0195 blockers fail.

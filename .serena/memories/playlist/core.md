...
- Session 06 persistence boundaries live in `mem:playlist/state`; Session 07 atomic save/durability, Sessions 08–09A discovery, Sessions 10A–10D runtime/media-open foundations, Sessions 11A–11C manual controller foundations, and Session 12 automatic lifecycle are complete. `playlist-core` remains serde/I/O-neutral; its new `queue::automatic` boundary owns opaque fixed-snapshot automatic traversal plans/tokens, including atomic shuffle RepeatQueue cycle commit and removal revalidation without admitting late rows. App policy is documented in `mem:app-egui/playlist-controller-s12`. Next allowed work is Session 12A only: active tombstone/removal/Undo; UI/config, persistence wiring, and discovery execution remain outside the completed scope.

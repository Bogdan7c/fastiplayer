...
- Session 06 persistence boundaries live in `mem:playlist/state`; Session 07 atomic save/durability, Sessions 08–09A discovery probe/manifest/executor/admission/readiness are complete. `playlist-core` itself remains serde/I/O-neutral. Session 10A app wake bridge and process-lifetime `PlaylistRuntime` shell is complete. Next allowed work is Session 10B only: secret-safe URL/service locator boundaries; controller, UI/config and app-player integration remain outside the completed scope.
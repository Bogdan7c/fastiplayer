...
- `ui-artwork-egui::playlist_toolbar` владеет шестью neutral hand-drawn vector glyphs `AddFiles`, `AddUrl`, `Sort`, `CurrentItem`, `Import` и `Clear`, плюс surface/focus geometry за `ArtworkPainter::playlist_toolbar_button(...)`. Import — стрелка в tray и characterization-тестом отличается от Add Files.
- `app-egui::ui::playlist::toolbar::icon_bar` владеет full-width row layout, stable hit areas/IDs, pointer/keyboard interaction, accessibility/tooltips, disabled reasons, sort/import popup и mapping в typed `PlaylistAction`; queue/domain mutation отсутствует. Первые четыре left axes и Clear anchor сохранены, Import добавлен пятым left slot после Current Item.
...
## Playlist header Undo artwork (2026-07-18)
- Нейтральная векторная Undo-рисовка выделена из toolbar в отдельный `ui-artwork-egui::undo_button`; facade — `ArtworkPainter::undo_button(...)`, vocabulary — `UndoButtonPaintState` и `UndoButtonStyle`.
- `PlaylistToolbarGlyph` больше не содержит Undo, а `PlaylistToolbarPaintState` не несёт `opacity`/`content_scale`: toolbar artwork снова описывает только пять постоянных glyph AddFiles/AddUrl/Sort/CurrentItem/Clear.
...
- `ArtworkPainter::playlist_row_outline(...)` остаётся neutral focus/insertion overlay boundary. Playback больше не использует тот же full-row контур: selection принадлежит row fill, focus/insertion — outline, confirmed active playback — отдельному left-edge marker-у.
- `ui-artwork-egui::PlaylistRowMarkerStyle` и `ArtworkPainter::playlist_row_marker(...)` — нейтральный boundary внутри существующего `playlist_row.rs`: artwork владеет безопасно clamped геометрией/скруглением/Painter primitive, а `app-egui::PlaylistRowStyle` задаёт skin tokens и playback semantics. Minimal skin: width 3 pt, vertical inset 4 pt, radius 1.5 pt, grayscale fill `(245,245,245,235)`.

...
## 2026-07-17 directory-aware main picker admission
- The main single-file picker now classifies its selected native path before constructing `InAppQueueReplacementIntent`. `PlaylistController::classify_local_file_selection` compares lexical parent paths without filesystem I/O and returns an existing committed Item ID only for an exact same-directory locator.

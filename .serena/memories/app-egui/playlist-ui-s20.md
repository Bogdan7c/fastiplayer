...
- `ui/playlist/toolbar/icon_bar.rs` owns only layout/interaction/accessibility/action mapping; `ui-artwork-egui` owns vector geometry; `PlayerSkin::playlist_toolbar_style()` owns visual tokens. The flow row remains 32 points high so the following queue summary does not move; button rects receive an 8-point optical Y offset into the existing egui inter-item spacing, which centers the controls between the header separator and summary without overlap. URL editor and operation status remain in the parent toolbar renderer.
...
- Focused headless regressions находятся в `ui/playlist/toolbar/icon_bar.rs` (равенство paint shapes + отсутствие actions) и `ui/playlist/status.rs` (нулевая разница высоты Available/TemporarilyBlocked). Полный `app-egui` suite: 671 PASS; strict Clippy, Rust 1.96 locked workspace check, fmt, refactor guardrails и diff check PASS.

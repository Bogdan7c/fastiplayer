...
- `symphonia-codec-aac`: removes the upstream AAC-LC `channels.count() > 2` complexity guard and owns coded-order → canonical-plane mapping. AAC element type + `element_instance_tag` selects the destination plane; for 5.1 coded `FC,FL,FR,RL,RR,LFE` becomes canonical `FL,FR,FC,LFE,RL,RR` (`[2,0,1,4,5,3]`). Config 3–7 and duplicate-element behavior have direct patch tests. This mapping is required before any downstream layout-aware downmix; removing it silently swaps dialogue/LFE/surround roles.

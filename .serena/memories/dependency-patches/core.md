...
- `cros-codecs`: production VA-API codec layer for AV1/VP9/H.264/H.265; its AV1 contract consumes one OBU per `decode()` call, so the first-party AV1 adapter must resubmit the exact remaining temporal-unit suffix. Local AV1 global-motion shear math retains checked i64 intermediate products through Round2Signed/Clip3 before narrowing, preventing valid coefficients from panicking and rejecting transforms that premature truncation would admit; full contract/evidence: `mem:investigations/av1-global-motion-overflow-2026-09-09`. Other local needs include H.265 Dolby Vision RPU / unspecified NAL type 48..63 acceptance, dynamic WPP entry point storage for 4K streams, H.265 seek/flush picture-order reset and `NoRaslOutputFlag` behavior for CRA/IRAP, RPS/DPB diagnostics, and `DecodedHandle::try_is_ready()` so VA query errors are not collapsed into `true` during suppressed-surface reclaim.
- `symphonia-format-isomp4`: MP4/fMP4 container patch for composition offsets (`ctts`/`trun`) so B-frames get presentation PTS, classic `stss` and fragmented effective-sample-flags sync-safe video seek, direct `sidx`+SAP indexed fragmented seek without linear `moof` scan, mandatory fragmented `tfdt` v0/v1 decode timing/gaps, honest fragmented duration authority, structural truncation distinct from clean EOF, `tkhd` display orientation tags, MP4 `colr`/`mdcv`/`clli` raw tags for neutral color/HDR metadata, and QuickTime/iOS PCM/LPCM one-frame sample coalescing to avoid tiny-packet starvation after seek. `sidx` indexing is codec/format-ID-neutral, bounded to direct source offsets, supports ordered startup/late indexes and muxed audio-first track order, and does not affect classic MP4. Video keyframe verification above the container remains packet-based; audio/non-video fragmented seek uses timestamp landing.

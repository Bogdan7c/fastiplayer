...
- Workspace split: `app-egui` shell/composition; `player-core` worker/session/scheduler; `audio-core` neutral audio decoder/output/clock contracts; `media-core`/`codec-core`/`video-core`/`video-backend-api`/`render-core`/`capability-core` contract crates; `source-core` byte access; `symphonia-demux` concrete demux; `service-youtube` yt-dlp adapter; `video-vaapi` VA-API decode; `render-wgpu` WGPU render; `audio` concrete Symphonia/Opus decoder and CPAL output backend; `desktop-integration` desktop controls adapter.

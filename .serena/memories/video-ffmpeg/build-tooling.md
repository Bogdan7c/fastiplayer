...
- Configure policy: `--enable-shared`, `--disable-static`, `--disable-programs`, `--disable-doc`, explicit `--disable-gpl`, `--disable-nonfree`, `--disable-autodetect`, required `libavcodec` + `libavutil`, disabled `libavformat`/`libavdevice`/`libavfilter`, disabled hwaccels/encoders/muxers/demuxers/protocols/devices/filters. FFmpeg 8.1.1 has no `--disable-postproc` configure option, so the tooling must not pass it.

...
- Focused tests: `cargo test -p player-core`; capability/codec policy tests: `cargo test -p capability-core -p codec-core`; render split checks: `cargo test -p render-wgpu-video`, `cargo test -p render-wgpu`, then `cargo check -p app-egui`.

...
- Focused tests: `cargo test -p player-core`; capability/codec policy tests: `cargo test -p capability-core -p codec-core`.
...
- Project docs mention `scripts/check-refactor-guardrails.py`, but current `scripts/` listing did not contain it during onboarding; verify before relying on that command.
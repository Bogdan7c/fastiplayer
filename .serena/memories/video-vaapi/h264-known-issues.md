...
## S27 ordinary Baseline capability (2026-07-22)
- Ordinary H.264 Baseline is now a distinct exact codec profile across capability report schema v7, VA probe, factory validation and cros-codecs SPS→VA mapping. Hardware support is conditional on exact `VAProfileH264Baseline`; lack of that row falls through normal capability selection to FFmpeg software rather than being mislabeled as Main/ConstrainedBaseline.
- Live debug proof for the explicit YouTube URL selected VA-API H.264 at 640x368 on AMD Radeon 780M, exported NV12 DMA-BUF and played AAC. The stream's runtime SPS may refine the container-level `avc1.42001E` requirement to a hardware-supported constrained form; do not infer that this GPU universally supports ordinary Baseline from that one stream.
- Focused `video-vaapi` suite: 137 tests pass; strict affected Clippy passes. Direct vendored cros-codecs test target remains blocked by its pre-existing incompatible dummy/test modules, while production cros-codecs code compiles and is covered through `video-vaapi`.

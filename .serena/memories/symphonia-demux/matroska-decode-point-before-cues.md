...
S28B добавил hermetic cue/no-cue Matroska/WebM corpus и подтвердил, что этот seek contract не меняется при finite ordered capability и `CodecState` lifecycle. Ordered input остаётся non-seekable; cue-aware seek доказывается только на seekable local/Range byte sources. Полная матрица: `mem:symphonia-demux/matroska-webm-s28b-2026-07-22`.
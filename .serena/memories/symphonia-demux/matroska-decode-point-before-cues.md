...
Focused coverage: unit tests in `symphonia_demuxer.rs` cover cue anchor selection, previous-cue retry, and too-far rescue. Fixture tests in `tests/h264_fixtures.rs` and `tests/h265_fixtures.rs` cover MKV targets 3s/5s/10s and assert a near cue/keyframe instead of old 5s backoff. Existing MOV/MP4 and VP9/WebM fixture tests should remain green.
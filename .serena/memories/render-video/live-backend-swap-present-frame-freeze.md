...
## Same-lineage pre-barrier checkpoint (2026-07-27)
- Same-item strong install must capture the old visual identity before authorization. Capturing `render_generation` in the post-`Installed` callback can observe the already-advanced new generation and leave freeze waiting forever.
- `PendingStrongLineageCommit::SameLineage` owns a boxed `BackendSwapVideoCheckpoint` containing the pre-barrier generation plus the complete cloned `CachedRenderablePresentFrame`. The complete frame keeps texture views and per-frame SDR/HDR metadata atomic across software/hardware pipeline changes.
- `capture_same_lineage_restore_before_barrier` refreshes old-instance state, validates exact active lineage, and records the checkpoint before authorization. `begin_post_installed_intent` consumes it exactly once and activates freeze only after `finish_media_open_terminal` returns exact `Installed`; pre-barrier cancellation/error drops it without touching active freeze state.
- Settings-driven backend rebuilds use the same capture/activate split and capture before `PlayerWorker::set_video_backend` mutates worker generation. Freeze still ends only when generation changes and the first new frame exists, or on audio-only/source replacement.
- Regression anchors: `state::video_backend::tests::pre_barrier_generation_releases_freeze_on_first_installed_frame`, `state::strong_media_open::startup_poll_tests::same_lineage_visual_checkpoint_crosses_install_barrier_in_order`, the four software/hardware candidate transitions, and renderer hardware/software SDR/HDR metadata matrix. Intel runtime validation of VP9/software -> H.264/VAAPI showed the first generation-1 NV12 DMA-BUF import and continuously rendered video instead of black output.

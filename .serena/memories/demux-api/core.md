...
- `DemuxFactory` публикует immutable descriptor: factory ID, container registrations и fixture IDs. После S28A2P каждый `DemuxContainerRegistration` владеет exact `DemuxInputCapabilities`; `DemuxFactoryDescriptor::input_capabilities()` вычисляет только factory-wide union для prefilter-а. После probe registry обязательно перепроверяет exact пару `(matched container, input capability)`, а planner проецирует capabilities из конкретной registration, поэтому ISO BMFF ordered support не протекает в WebM/Ogg/WAV.

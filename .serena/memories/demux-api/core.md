...
- Оба варианта `ProgressiveSeekCommand` всегда владеют request-scoped cancellation token; внутренний `cancellation()` поэтому является total borrowed boundary, а не `Option`. Receipted seek accounting принадлежит progressive queue: каждый принятый fence обязан получить ровно один terminal outcome, а bounded capacity освобождается только при `poll_receipt`. Поэтому sync preview, вытесняющий pending `Receipted`, под тем же queue mutex делает `take`/cancel и ставит exact fence + `Superseded` в FIFO pending receipts; active command остаётся worker-owned и не terminalize-ится control path-ом. Функциональный regression test: `src/progressive/tests/sync_preview_receipt.rs` блокирует старый read, проверяет полный capacity до poll, exact `Superseded`, newest downstream packet PTS и повторное использование capacity.

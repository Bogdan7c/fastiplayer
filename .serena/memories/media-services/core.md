...
- Production S15A extended capabilities: exact `Ftp`/`Ftps` зарегистрированы после S37 (`mem:media-services/progressive-ftp-s37-2026-07-25`). RTMP variants остаются dormant до S39 и возвращают typed `ImplementedProviderUnavailable`. Exact per-scheme registration без alias expansion сохраняется.

...
- Video accepts only F2 `Admitted` and publishes ordinary ordered media. Audio maps `Admitted` to `PacketPresentationWindow::Unbounded`; `PendingExactAudioClipping` keeps reconstructed bytes unchanged and publishes an exact bounded window using F2's authoritative reconstructed track ID, checked i64 tick conversion and native timebase. F3A/F3B own packet transport and decoded-PCM clipping.

...
S12 no-suffix continuation consumer: tests/live_runtime/no_suffix.rs reads the final actual initial Opus packet, enables a newer publishTime with unchanged media segment inventory, then waits for TWO sequential refreshed MPD responses. The second request can start only after the first refresh commit, unlike a single response-send notification. A subsequent demux read must return TemporarilyUnavailable without replay/refetch or false EOF; cancellation and detached refresh-owner shutdown remain bounded. Same-inventory accepted refresh therefore exercises component open_continuation_after(None) and runtime install_snapshot_continuation(false) deterministically. No production changes.

...
- Correction 2026-08-09 (implemented): pinned `yt-dlp 2026.07.04` сериализует отдельный `cookies` field не как готовый request `Cookie` header, а как flattened scoped cookie records (`name=value` плюс `Domain`/`Path`/`Secure`/`Expires`/`Version`). `service-ytdlp::candidate::request_material::cookies` теперь владеет bounded exact grammar и различает `RequestHeader`/`ScopedSeeds`; case-insensitive `Cookie` внутри `http_headers` остаётся request-header формой, а mixed/conflicting material fail-closed. `source-core::HttpCookieSeed` переносит каждый record отдельно в per-source jar с RFC Domain/Path/Secure/expiry matching. Это устранило pre-I/O `UnsupportedTransportReason::RequestMaterial` для реального Wikimedia Ogg. Полный Ogg/prefetch/demux audit: `mem:media-services/ytdlp-scoped-cookies-prefetch-ogg-2026-08-09`.

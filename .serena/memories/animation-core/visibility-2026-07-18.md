...
- Тонкий app-адаптер находится в `app-egui::ui::animation`: `VisibilityAnimationSpec`, `VisibilityTarget` и typed `VisibilityMotion::{Standard, Reduced}`. Он владеет stable egui ID, target, timing/repaint и мгновенным reduced-motion режимом, но не знает о конкретном widget/domain.
...
- Первый consumer — Playlist Undo: 180 ms, opacity `0→1`, scale `0.80→1.00`; выход является точным обратным переходом, reduced motion мгновенный.

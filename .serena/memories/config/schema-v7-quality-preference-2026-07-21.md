...
- `fastiplayer_config::YtDlpConfig::preferred_video_height: Option<PreferredVideoHeight>` — единственный durable global quality knob. `None` означает обычный BestPlayable. Config-owned newtype принимает только `1..=16_384`; TOML сериализует его scalar-числом. Config crate не зависит от `web-media-core`.

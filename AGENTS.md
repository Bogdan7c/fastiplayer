...
## Обязательная разгрузка исследования через Luna

Основной агент обязан беречь свой контекст для требований пользователя, архитектурных решений, реализации и финальной проверки. Широкое исследование репозитория по умолчанию выполняет project-scoped субагент `luna_worker` из `.codex/agents/luna_worker.toml`.

Перед самостоятельным широким чтением или поиском основной агент обязан сначала делегировать `luna_worker` отдельную bounded-задачу, если заранее не известны точные файлы, символы и полный путь зависимостей. Это обязательно для:
...
Для независимых направлений исследования основной агент обязан запускать отдельные bounded-задачи параллельно, пока это укладывается в доступный лимит субагентов. Нельзя поручать двум субагентам запись в пересекающиеся файлы; `luna_worker` всегда остаётся read-only.

После handoff основной агент не должен повторять широкое исследование и загружать те же файлы целиком. Он читает только указанные Luna точные символы, ссылки и небольшие фрагменты, необходимые для проверки высокорисковых выводов или устранения противоречий.
...
- оценку полноты доказательств Luna;
- финальный self-review и запуск достаточных проверок.

Пропустить делегирование разрешено только когда задача не относится к коду/репозиторию либо пользователь уже указал точный файл и символ, а поиск ссылок и соседних владельцев не требуется. Если `luna_worker` недоступен, не запускается или не поддерживается текущей сессией, основной агент обязан сразу сообщить об этом в commentary и использовать доступного read-only субагента с тем же bounded-заданием. Молча переходить к широкому самостоятельному исследованию запрещено.

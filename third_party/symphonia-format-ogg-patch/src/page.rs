...
impl PageReader {
        ...
        B: ReadBytes + SeekBuffered,
    {
        ...
        let page_start = sync_pos.saturating_sub(OGG_PAGE_MARKER.len() as u64);
    ...
    pub fn first_packet(&self) -> Option<&[u8]> {
        self.packet_lens
            .first()
            .map(|&len| &self.page_buf[..usize::from(len)])
    }
    ...
    pub fn page(&self) -> Page<'_> {
        assert!(
            self.page_buf_len <= 255 * 255,
            "ogg pages are <= 65025 bytes"
        );

        Page {
            header: self.header,
            source_byte_offset: self.page_start,

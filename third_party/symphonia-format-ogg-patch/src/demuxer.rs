...
impl OggReader {
    ...
    fn target_timestamp_and_serial(&self, to: &SeekTo) -> Result<(u64, u32)> {
        match *to {
    ...
    fn scan_to_required_packet(&mut self, serial: u32, required_ts: u64) -> Result<SeekedTo> {
        let actual_ts = loop {
    ...
    fn seek_with_byte_offset_hint(
        ...
        self.scan_to_required_packet(serial, required_ts)
    }
    ...
    fn start_new_physical_stream(&mut self) -> Result<()> {
        ...
        for (&serial, stream) in streams.iter() {
            ...
            self.tracks
                .push(Track::new(serial, stream.codec_params().clone()));
        }
...
impl FormatReader for OggReader {
    ...
    fn seek(&mut self, _mode: SeekMode, to: SeekTo) -> Result<SeekedTo> {
        let (required_ts, serial) = self.target_timestamp_and_serial(&to)?;
    ...
    fn seek_with_hint(&mut self, mode: SeekMode, to: SeekTo, hint: SeekHint) -> Result<SeekedTo> {
        ...
        if let Some(byte_offset) = hint.byte_offset {
            match self.seek_with_byte_offset_hint(serial, required_ts, byte_offset) {
                Ok(seeked_to) => return Ok(seeked_to),
                Err(error) => debug!("ogg byte-offset seek hint failed: {}", error),
            }
        }

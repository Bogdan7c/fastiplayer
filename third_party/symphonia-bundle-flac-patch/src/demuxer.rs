...
impl FlacReader {
    ...
    fn target_timestamp(&self, to: &SeekTo) -> Result<u64> {
        ...
        match *to {
            ...
            SeekTo::Time { time, .. } => {
                if let Some(sample_rate) = params.sample_rate {
                    Ok(TimeBase::new(1, sample_rate).calc_timestamp(time))
                } else {
    ...
    fn check_timestamp_range(&self, ts: u64) -> Result<()> {
        let params = &self.tracks[0].codec_params;
    ...
    fn seek_forward_from_current_position(&mut self, ts: u64) -> Result<SeekedTo> {
        let packet = loop {
    ...
    fn seek_with_byte_offset_hint(&mut self, ts: u64, byte_offset: u64) -> Result<SeekedTo> {
        if !self.reader.is_seekable() {
            return seek_error(SeekErrorKind::Unseekable);
        }

        self.reader.seek(SeekFrom::Start(byte_offset))?;
        self.parser.resync(&mut self.reader)?;
        self.seek_forward_from_current_position(ts)
    }
}
...
impl FormatReader for FlacReader {
    ...
    fn seek_with_hint(&mut self, mode: SeekMode, to: SeekTo, hint: SeekHint) -> Result<SeekedTo> {
        ...
        if let Some(byte_offset) = hint.byte_offset {
            match self.seek_with_byte_offset_hint(ts, byte_offset) {
                Ok(seeked_to) => return Ok(seeked_to),
                Err(error) => debug!("flac byte-offset seek hint failed: {}", error),
            }
        }

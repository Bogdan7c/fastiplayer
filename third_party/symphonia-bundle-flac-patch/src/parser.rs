...
impl Fragment {
    ...
    fn new(data: Box<[u8]>) -> Self {
        ...
        Self {
            ...
            footer,
            crc_match,
            state: FragmentState { crc16, total_len },
        }
    }
...
impl PacketParser {
    ...
    where
        B: ReadBytes + SeekBuffered,
    {
        let source_byte_offset = reader.pos();
        ...
        Ok(
            Packet::new_from_boxed_slice(0, parsed.sync.ts, parsed.sync.dur, parsed.buf)
    ...
    pub fn reset(&mut self, info: StreamInfo) {
        let max_frame_size = if info.frame_byte_len_max > 0 {
            Some(info.frame_byte_len_max as usize)
        } else {
            None
        };

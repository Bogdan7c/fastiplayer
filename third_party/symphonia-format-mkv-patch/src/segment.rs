...
impl Element for CueTrackPositionsElement {
    ...
    fn read<B: ReadBytes>(reader: &mut B, header: ElementHeader) -> Result<Self> {
        ...
        Ok(Self {
            track: track.ok_or(Error::DecodeError(
                "mkv: missing track in cue track positions",
            ))?,
            cluster_position: pos.ok_or(Error::DecodeError(
                "mkv: missing position in cue track positions",
            ))?,
        })
    }
}
...
pub(crate) struct BlockGroupElement {
    ...
    pub(crate) has_reference_block: bool,
}
        ...
        while let Some(header) = it.read_header()? {
            match header.etype {
                ...
                ElementType::ReferenceBlock => {
                    let _reference_timestamp = it.read_data()?;
...
impl Element for SimpleTagElement {
    ...
    fn read<B: ReadBytes>(reader: &mut B, header: ElementHeader) -> Result<Self> {
        ...
        Ok(Self {
            name: name
                .ok_or(Error::DecodeError("mkv: missing tag name"))?
                .into_boxed_str(),

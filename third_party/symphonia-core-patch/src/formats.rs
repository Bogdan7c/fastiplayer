...
/// Additional container-level hints a `FormatReader` may use to accelerate seeking.
///
/// Hints are optional and advisory. A format reader that cannot safely use a hint must ignore it
/// and fall back to the normal time/timestamp seek path.
#[derive(Copy, Clone, Debug, Default, Eq, PartialEq)]
pub struct SeekHint {
    /// Absolute byte offset in the underlying media source.
    ///
    /// The offset must point to a safe container resynchronization point at or before the requested
    /// target. For formats with nested structures this normally means an enclosing packet/page/
    /// cluster boundary, not arbitrary codec payload bytes.
    pub byte_offset: Option<u64>,
}

impl SeekHint {
    /// Creates an empty seek hint.
    pub const fn none() -> Self {
        Self { byte_offset: None }
    }

    /// Creates a seek hint with an absolute source byte offset.
...
pub trait FormatReader: Send + Sync {
    ...
    /// Seek using optional container-level hints.
    ///
    /// The default implementation preserves the existing behaviour for all formats. Individual
    /// demuxers can override this method when they can validate and use a hint safely.
...
pub struct Packet {
    ...
    /// The packet buffer.
    pub data: Box<[u8]>,
    /// Absolute byte offset of a safe container resynchronization point for this packet.
    pub source_byte_offset: Option<u64>,
    /// Container-level keyframe flag, when the format exposes one.
...
impl Packet {
    ...
    /// Attach a safe source byte offset to the packet.
    pub fn with_source_byte_offset(mut self, source_byte_offset: u64) -> Self {
        self.source_byte_offset = Some(source_byte_offset);
        self
    }

    /// Attach a container-level keyframe flag to the packet.
    ...
    impl SeekIndex {
        ...
        pub fn insert(&mut self, ts: u64, byte_offset: u64, n_frames: u32) {
            ...
            let (last_ts, last_offset) = self
                .points
                .last()
                .map_or((u64::MAX, u64::MAX), |p| (p.frame_ts, p.byte_offset));

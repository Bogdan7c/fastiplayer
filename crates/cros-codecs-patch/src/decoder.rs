...
/// CPU-readable decoded NV12 image copied from a decoder-owned surface.
///
/// This is used by applications that cannot reliably decode directly into external
/// DMA-BUF/GBM memory on a given driver.
pub struct DecodedNv12Image {
    /// Coded width of the image.
    pub width: u32,
    /// Coded height of the image.
    pub height: u32,
    /// Y plane bytes, including row padding according to `y_stride`.
    pub y_plane: Vec<u8>,
    /// Interleaved UV plane bytes, including row padding according to `uv_stride`.
    pub uv_plane: Vec<u8>,
    /// Y plane row pitch in bytes.
    pub y_stride: u32,
...
pub trait DecodedHandle {
    ...
    /// Optionally returns a CPU-readable NV12 copy from the backend's native decoded surface.
    ///
    /// Backends that only expose client-provided output memory can keep the default `None`.

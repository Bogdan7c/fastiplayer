...
impl PlayerSession {
    ...
    fn seek_mode_for_scrub_commit(&self, generation: ScrubGeneration) -> crate::SeekMode {
        self.latest_scrub_request
            .filter(|latest_intent| latest_intent.generation == generation)
...
mod tests {
    ...
    fn active_seek_drains_target_frame_when_present_queue_is_full() {
        ...
        session
            .pipeline
            .video_frame_queue
            .push_back(decoded_frame_for_tests(Duration::from_secs(5), 5));
    ...
    fn active_seek_long_preroll_drain_keeps_present_queue_bounded() {
        ...
        assert!(session.seek_commit().is_none());
        assert!(session.pipeline.video_frame_queue.len() <= 2);
        assert_eq!(
            session
                .pipeline
                .present_video_frame

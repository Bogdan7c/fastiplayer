...
fn apply_composition_offset(decode_ts: u64, composition_offset: i64) -> Result<u64> {
    if composition_offset >= 0 {
        let offset = composition_offset as u64;
        decode_ts
            .checked_add(offset)
            .ok_or(Error::DecodeError("isomp4: composition timestamp overflow"))
    } else {
        let offset = composition_offset.unsigned_abs();
        decode_ts
            .checked_sub(offset)
            .ok_or(Error::DecodeError("isomp4: negative composition timestamp"))
    }
}
    ...
    fn sample_timing(&self, track_num: usize, sample_num: u32) -> Result<Option<SampleTiming>> {
        ...
        for trun in traf.truns.iter() {
            ...
            if sample_num_rel < trun.sample_count {
                ...
                return Ok(Some(SampleTiming { pts, dts, dur }));
            }
    ...
    fn sample_data(
        ...
        for trun in traf.truns.iter() {
            ...
            if sample_num_rel < trun.sample_count {
                ...
                return Ok(SampleDataDesc {
                    base_pos: trun_offset,
                    size,
                    offset,
                });
            }
...
impl StreamSegment for MoovSegment {
    ...
    fn sample_timing(&self, track_num: usize, sample_num: u32) -> Result<Option<SampleTiming>> {
        ...
        if let Some((dts, dur)) = timing {
            let composition_offset = trak
                .mdia
                .minf
                .stbl
                .ctts
                .as_ref()
                .and_then(|ctts| ctts.find_offset_for_sample(sample_num))
                .unwrap_or(0);
            let pts = apply_composition_offset(dts, composition_offset)?;

            Ok(Some(SampleTiming { pts, dts, dur }))
        } else {
            Ok(None)
        }
    }

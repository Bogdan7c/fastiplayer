...
/// Возвращает начало payload-а единственного top-level `mdat` в test media segment-е.
fn mdat_payload_offset(segment_bytes: &Bytes, preceding_bytes: usize) -> u64 {
    const BASIC_BOX_HEADER_BYTES: usize = 8;
    let mut box_cursor = 0_usize;
    while box_cursor + BASIC_BOX_HEADER_BYTES <= segment_bytes.len() {
        let box_length = u32::from_be_bytes(
            segment_bytes[box_cursor..box_cursor + 4]
                .try_into()
                .expect("ISO box size занимает четыре байта"),
        ) as usize;
        assert!(box_length >= BASIC_BOX_HEADER_BYTES);
        let box_end = box_cursor
            .checked_add(box_length)
            .expect("fixture box boundary не переполняется");
        assert!(box_end <= segment_bytes.len());
        if &segment_bytes[box_cursor + 4..box_cursor + BASIC_BOX_HEADER_BYTES] == b"mdat" {
            let payload_offset = preceding_bytes
                .checked_add(box_cursor)
                .and_then(|offset| offset.checked_add(BASIC_BOX_HEADER_BYTES))
                .expect("fixture source offset не переполняется");
            return u64::try_from(payload_offset).expect("fixture offset помещается в u64");
        }
        box_cursor = box_end;
    }
    panic!("generated media segment должен содержать top-level mdat");
}
...
fn ordered_fmp4_opens_without_hint_and_reads_multiple_media_fragments() {
    ...
    let expected_packet_offsets = [
        mdat_payload_offset(&first_media, init.len()),
        mdat_payload_offset(&second_media, init.len() + first_media.len()),
    ];
    ...
    for _ in 0..16 {
        match demuxer.next_event().expect("read ordered fMP4") {
            DemuxReadEvent::Packet(packet) => {
                ...
                assert_eq!(
                    packet.byte_offset,
                    Some(expected_packet_offsets[packet_index]),
                    "offset обязан указывать на начало sample в виртуально склеенном input-е"
                );
            }

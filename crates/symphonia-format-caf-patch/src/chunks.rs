...
impl Chunk {
    ...
    ///   - or, для audio data на forward-only source, в начале payload независимо
    ///     от того, объявлена ли длина chunk-а.
    ///
...
impl PacketTable {
    pub fn read(
        ...
        let mut current_frame = Timestamp::from(-i64::from(if priming_frames > 0 {
            priming_frames
        } else {
            0
        }));

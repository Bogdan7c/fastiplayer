...
const CAF_FORMAT_INFO: FormatInfo = FormatInfo {
    format: FORMAT_ID_CAF,
    short_name: "caf",
    ...
    fn next_packet(&mut self) -> Result<Option<Packet>> {
        match &mut self.packet_info {
            PacketInfo::FixedAudioPacket {
                ...
                let buf = if self.data_len.is_some() {
                    self.reader.read_boxed_slice_exact(bytes_to_read as usize)?
                } else {
                    self.reader.read_boxed_slice(bytes_to_read as usize)?
                };

                Ok(Some(Packet::new(0, pts, dur, buf)))
            }
            PacketInfo::VariableAudioPacket {
                packets,
                current_packet_index,
            } => {
                if let Some(packet) = packets.get(*current_packet_index) {
                    *current_packet_index += 1;
                    let buffer = self.reader.read_boxed_slice_exact(packet.size as usize)?;
                    Ok(Some(Packet::new(
                        0,
                        packet.start_frame,
                        packet.frames,
...
impl<'s> CafReader<'s> {
    ...
    fn read_chunks(&mut self) -> Result<Track> {
        ...
        let source_is_seekable = self.reader.is_seekable();
        ...
        loop {
            let forward_data_ready = match Chunk::read(&mut self.reader, &audio_desc)? {
...
mod tests {
    ...
    /// Генерирует fixed-packet mono PCM CAF без внешнего media asset-а.
    fn generated_pcm_caf(declared_sample_bytes: usize, actual_sample_bytes: usize) -> Vec<u8> {
        let mut bytes = Vec::with_capacity(68 + actual_sample_bytes);
        ...
        bytes.extend_from_slice(&(4_i64 + declared_sample_bytes as i64).to_be_bytes());
        bytes.extend_from_slice(&0_u32.to_be_bytes());
        bytes.extend((0..actual_sample_bytes).map(|index| index as u8));
        bytes
    }

    /// Строит seekable reader поверх exact generated bytes.
    fn seekable_reader(bytes: Vec<u8>) -> CafReader<'static> {
        let source = MediaSourceStream::new(Box::new(Cursor::new(bytes)), Default::default());
        CafReader::try_new(source, FormatOptions::default()).expect("seekable generated CAF")
    }

    /// Строит forward-only reader с неизвестной physical length.
    fn streaming_reader(bytes: Vec<u8>) -> CafReader<'static> {
        let source = ReadOnlySource::new(Cursor::new(bytes));
        let source = MediaSourceStream::new(Box::new(source), Default::default());
        CafReader::try_new(source, FormatOptions::default()).expect("streaming generated CAF")
    }

    /// Existing seekable scan по-прежнему возвращается от EOF к началу data payload.
    #[test]
    fn seekable_caf_scan_and_packet_regression() {
        let mut reader = seekable_reader(generated_pcm_caf(64, 64));
        assert!(reader.reader.is_seekable());
        assert_eq!(reader.reader.pos(), reader.data_start_pos);
        assert_eq!(reader.tracks[0].duration, Some(Duration::from(32_u64)));
        let packet = reader
            .next_packet()
            .expect("seekable packet read")
            .expect("packet");
        assert_eq!(packet.dur, Duration::from(32_u64));
        assert_eq!(packet.data.len(), 64);
        assert!(reader.next_packet().expect("seekable clean EOS").is_none());
    }

    /// Forward-only scan останавливается перед data и сохраняет payload для packet reader-а.
    #[test]
    fn streaming_caf_opens_reads_packet_and_reaches_clean_eos() {
        let mut reader = streaming_reader(generated_pcm_caf(64, 64));
        assert!(!reader.reader.is_seekable());
        assert_eq!(reader.reader.pos(), reader.data_start_pos);
        assert_eq!(reader.tracks[0].duration, Some(Duration::from(32_u64)));
        let packet = reader
            .next_packet()
            .expect("streaming packet read")
            .expect("packet");
        assert_eq!(packet.pts, Timestamp::ZERO);
        assert_eq!(packet.dur, Duration::from(32_u64));
        assert_eq!(packet.data.len(), 64);
        assert!(reader.next_packet().expect("streaming clean EOS").is_none());
    }

    /// Declared data payload truncation остаётся structural read failure, а не short packet/EOS.
    #[test]
    fn streaming_caf_rejects_truncated_declared_data() {
        let mut reader = streaming_reader(generated_pcm_caf(64, 63));
        let error = reader
            .next_packet()
            .expect_err("truncated CAF data должен fail-нуться");
        assert!(matches!(
            error,
            Error::IoError(ref source) if source.kind() == ErrorKind::UnexpectedEof
        ));
    }

    /// Truncated chunk header до data не становится допустимым завершением initial scan-а.
    #[test]
    fn streaming_caf_rejects_truncated_chunk_header() {
        let mut bytes = generated_pcm_caf(64, 64);
        bytes.truncate(14);
        let source = ReadOnlySource::new(Cursor::new(bytes));
        let source = MediaSourceStream::new(Box::new(source), Default::default());
        let error = match CafReader::try_new(source, FormatOptions::default()) {
            Ok(_) => panic!("truncated CAF chunk header должен fail-нуться"),
            Err(error) => error,
        };
        assert!(matches!(
            error,

...
#[derive(Debug)]
pub enum EbmlError {
    /// Physical EOF встретился до чтения первого байта следующего element header-а.
    EndOfDocument,
    ...
    MaximumDepthReached,
    /// Заголовок элемента обрезан физическим концом документа.
    TruncatedElementHeader,
    /// Объявленный payload элемента выходит за физический конец документа.
    ElementBeyondDocumentEnd,
...
impl<R: ReadEbml, S: EbmlSchema> EbmlIterator<R, S> {
    ...
    pub(crate) fn next_header(&mut self) -> Result<Option<&EbmlElementHeader<S>>> {
        ...
        // Физический конец известного документа — clean EOF только на точной границе.
        // Один оставшийся байт не может содержать минимальный EBML header и является truncation.
        if let Some(document_end) = self.len {
            let position = self.reader.pos();
            if position == document_end {
                return Ok(None);
            }
            if position < document_end && document_end - position < EbmlElementHeader::<S>::MIN_SIZE
            {
                return Err(EbmlError::TruncatedElementHeader);
            }
        }
        ...
        if let Some(parent_end) = parent_end {
            ...
            if pos == parent_end {
                ...
                // A declared parent with a trailing partial header is structurally truncated.
                return Err(EbmlError::TruncatedElementHeader);
            }
        }
        ...
        let header = match EbmlElementHeader::read(&mut self.reader, depth, &self.schema) {
            Err(EbmlError::EndOfDocument) if parent_end.is_none() => return Ok(None),
            Err(EbmlError::EndOfDocument) => return Err(EbmlError::TruncatedElementHeader),
            result => result?,
        };

        // У master element с unknown size нет parent_end, поэтому отдельно проверяем
        // заявленный child payload против фактической длины источника.
        if let (Some(document_end), Some(child_end)) = (self.len, header.end()) {
            if child_end > document_end {
                return Err(EbmlError::ElementBeyondDocumentEnd);
            }
        }
...
fn read_element_id<R: ReadBytes>(reader: &mut R) -> Result<(u32, u8)> {
    ...
    let byte = match reader.read_byte() {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::UnexpectedEof => {
            return Err(EbmlError::EndOfDocument);
        }
        Err(error) => return Err(EbmlError::IoError(error)),
    };
...
mod tests {
    ...
    use super::{EbmlError, EbmlIterator, read_element_id, read_signed_vint, read_unsigned_vint};

    fn iterator(bytes: &[u8]) -> EbmlIterator<MediaSourceStream<'static>, MkvSchema> {
        let length = bytes.len() as u64;
        let stream =
            MediaSourceStream::new(Box::new(Cursor::new(bytes.to_vec())), Default::default());
        EbmlIterator::new(stream, MkvSchema, Some(length))
    }

    /// Создаёт forward-only модель, где физическая длина документа заранее неизвестна.
    fn streaming_iterator(bytes: &[u8]) -> EbmlIterator<MediaSourceStream<'static>, MkvSchema> {
        let stream =
            MediaSourceStream::new(Box::new(Cursor::new(bytes.to_vec())), Default::default());
        EbmlIterator::new(stream, MkvSchema, None)
    }
    ...
    #[test]
    fn exact_document_end_is_clean_but_partial_header_is_truncation() {
        let clean = [0xec, 0x80];
        let mut clean_iter = iterator(&clean);
        assert_eq!(
            clean_iter.next_header().unwrap().unwrap().element_type(),
            MkvElement::Void
        );
        assert!(clean_iter.next_header().unwrap().is_none());

        let truncated = [0xec, 0x80, 0xec];
        let mut truncated_iter = iterator(&truncated);
        assert_eq!(
            truncated_iter.next_header().unwrap().unwrap().element_type(),
            MkvElement::Void
        );
        assert!(matches!(
            truncated_iter.next_header(),
            Err(EbmlError::TruncatedElementHeader)
        ));
    }

    #[test]
    fn declared_payload_beyond_document_end_is_structural_truncation() {
        let truncated = [0xec, 0x85];
        let mut iter = iterator(&truncated);
        assert!(matches!(
            iter.next_header(),
            Err(EbmlError::ElementBeyondDocumentEnd)
        ));
    }

    #[test]
    fn streaming_eof_inside_declared_parent_is_structural_truncation() {
        // Segment обещает четыре payload-байта, но источник заканчивается после
        // двухбайтового Void. Общая длина неизвестна, зато parent boundary известна.
        let truncated = [0x18, 0x53, 0x80, 0x67, 0x84, 0xec, 0x80];
        let mut iter = streaming_iterator(&truncated);
        assert_eq!(
            iter.next_header().unwrap().unwrap().element_type(),
            MkvElement::Segment
        );
        iter.push_element().unwrap();
        assert_eq!(
            iter.next_header().unwrap().unwrap().element_type(),
            MkvElement::Void
        );
        assert!(iter.read_binary().unwrap().is_empty());
        assert!(matches!(
            iter.next_header(),
            Err(EbmlError::TruncatedElementHeader)
        ));

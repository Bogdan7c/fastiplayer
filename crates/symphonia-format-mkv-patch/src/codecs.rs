...
/// Применяет Matroska `CodecState` как полную замену исходного `CodecPrivate`.
///
/// Контейнер обязан опубликовать обновлённые параметры до первого packet-а,
/// который использует новое состояние. Сам lifecycle-сигнал отправляет demuxer.
pub(crate) fn apply_codec_state(
    track: &mut symphonia_core::formats::Track,
    state: Box<[u8]>,
) -> Result<()> {
    let Some(codec_params) = track.codec_params.as_mut() else {
        // Неизвестный codec всё равно остаётся корректным demux track-ом. Для него
        // нечего обновлять, но demuxer всё равно сообщит ResetRequired вызывающему коду.
        return Ok(());
    };

    match codec_params {
        CodecParameters::Audio(params) => {
            let extra_data = audio_extra_data_from_codec_private(params.codec, &state)?;
            params.extra_data = Some(extra_data);
        }
        CodecParameters::Video(params) => {
            let extra_data_id = video_extra_data_id(params.codec);
            params.extra_data.retain(|extra_data| extra_data.id != extra_data_id);
            params.extra_data.push(VideoExtraData { id: extra_data_id, data: state });
        }
        CodecParameters::Subtitle(params) => {
            params.extra_data = Some(state);
        }
        _ => return decode_error("mkv: unsupported codec state parameter type"),
    }
...
fn make_audio_codec_params(
    ...
    if let Some(codec_private) = track.codec_private {
        let extra_data = audio_extra_data_from_codec_private(id, &codec_private)?;
...
fn make_video_codec_params(
    ...
    if let Some(codec_private) = track.codec_private {
        let extra_data_id = video_extra_data_id(id);
...
fn audio_extra_data_from_codec_private(
    ...
    match codec_id {
        ...
        _ => Ok(codec_private.into()),
    }
}

fn video_extra_data_id(codec_id: VideoCodecId) -> symphonia_core::codecs::video::VideoExtraDataId {
    match codec_id {
...
mod tests {
    ...
    use symphonia_core::codecs::video::well_known::CODEC_ID_VP9;
    use symphonia_core::codecs::video::well_known::extra_data::{
        VIDEO_EXTRA_DATA_ID_DOLBY_VISION_CONFIG, VIDEO_EXTRA_DATA_ID_VP9_DECODER_CONFIG,
    };
    use symphonia_core::codecs::{CodecParameters, video::VideoCodecParameters};
    use symphonia_core::formats::Track;

    use super::apply_codec_state;

    #[test]
    fn codec_state_replaces_codec_private_without_dropping_other_video_extra_data() {
        let mut video = VideoCodecParameters { codec: CODEC_ID_VP9, ..Default::default() };
        video.add_extra_data(VideoExtraData {
        ...
        track.with_codec_params(CodecParameters::Video(video));

        apply_codec_state(&mut track, Box::from([3, 4])).expect("codec state");

        let video = track.codec_params.as_ref().and_then(CodecParameters::video).unwrap();
        assert_eq!(video.extra_data.len(), 2);
        assert!(video.extra_data.iter().any(|extra_data| {
            extra_data.id == VIDEO_EXTRA_DATA_ID_VP9_DECODER_CONFIG
                && extra_data.data.as_ref() == [3, 4]
        }));
        assert!(video.extra_data.iter().any(|extra_data| {
            extra_data.id == VIDEO_EXTRA_DATA_ID_DOLBY_VISION_CONFIG
        }));
    }
}

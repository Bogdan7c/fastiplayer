...
use symphonia_core::errors::{
    Error, Result, SeekErrorKind, reset_error, seek_error, unsupported_error,
};
...
use crate::codecs::{apply_codec_state, make_track_codec_params};
use crate::ebml::{EbmlElementInfo, EbmlError, EbmlIterator, EbmlSchema, ReadEbml};
use crate::lacing::{Frame, block_track_num, extract_frames};
...
pub struct MkvReader<'s> {
    ...
    /// `CodecState` уже установлен в track list, но lifecycle reset ещё не наблюдал caller.
    reset_required_before_frames: bool,
}
...
impl<'s> MkvReader<'s> {
    pub fn try_new(mss: MediaSourceStream<'s>, opts: FormatOptions) -> Result<Self> {
        ...
        Ok(Self {
            ...
            current_cluster,
            frames: VecDeque::new(),
            reset_required_before_frames: false,
        })
    }
            ...
            Some(child) => {
                match child.element_type() {
                    ...
                    block_type @ (MkvElement::SimpleBlock | MkvElement::BlockGroup) => {
                        ...
                        // Get block data, duration, and optional decoder configuration change.
                        let (data, duration, codec_state) = match block_type {
                            ...
                            MkvElement::BlockGroup => {
                                ...
                                (group.data, group.duration, group.codec_state)
                            }
                            _ => unreachable!(),
                        };

                        // Сначала полностью валидируем Block во временной очереди. Так malformed
                        // lacing не оставит частично обновлённые codec params или packet queue.
                        let mut parsed_frames = VecDeque::new();
                        if !extract_frames(
                            &data,
                            duration,
                            cluster_ts,
                        ...
                        if let Some(codec_state) = codec_state {
                            let track_num = block_track_num(&data)?;
                            let track = self
                                .tracks
                                .iter_mut()
                                .find(|track| track.id == track_num)
                                .ok_or(Error::DecodeError(
                                    "mkv: codec state references an invalid track",
                                ))?;
                            apply_codec_state(track, codec_state)?;
                            self.reset_required_before_frames = true;
                        }
...
impl FormatReader for MkvReader<'_> {
    ...
    fn next_packet(&mut self) -> Result<Option<Packet>> {
        loop {
            if self.reset_required_before_frames {
                self.reset_required_before_frames = false;
                return reset_error();
            }
...
impl From<EbmlError> for Error {
    fn from(value: EbmlError) -> Self {
        ...
        let msg = match value {
            EbmlError::IoError(err) if err.kind() == std::io::ErrorKind::UnexpectedEof => {
                "mkv (ebml): truncated declared element data"
            }
            EbmlError::IoError(err) => return Error::IoError(err),
            EbmlError::EndOfDocument => "mkv (ebml): unexpected document end",
            ...
            EbmlError::TruncatedElementHeader => "mkv (ebml): truncated element header",
            EbmlError::ElementBeyondDocumentEnd => {
                "mkv (ebml): element payload extends beyond document end"
            }

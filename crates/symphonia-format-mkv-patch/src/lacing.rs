...
/// Читает Matroska TrackNumber из Block без разбора codec payload/lacing.
pub(crate) fn block_track_num(block: &[u8]) -> Result<u32> {
    let mut reader = BufReader::new(block);
    Ok(read_unsigned_vint(&mut reader)? as u32)
}

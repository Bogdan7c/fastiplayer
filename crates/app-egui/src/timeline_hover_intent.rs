...
use crate::ui::timeline::{TimelineHoverIntent, TimelineHoverTarget};
...
pub(crate) struct TimelineHoverFrameOutcome {
    ...
    pub(crate) visual_presentation_target: Option<TimelineHoverTarget>,
...
impl TimelineHoverIntentState {
    ...
    fn apply_frame_intent(
        ...
        match intent {
            Some(TimelineHoverIntent::Target(target)) => {
                self.apply_target(target, visual_preview_enabled)
            }
    ...
    fn apply_target(
        ...
        self.apply_visual_preview_target(target, visual_preview_enabled, &mut outcome);
        outcome
    }
    ...
    fn apply_visual_preview_target(
        ...
        if visual_preview_enabled {
            if self.preview_slot != TimelineHoverPreviewSlot::Pending(target) {
                self.preview_slot = TimelineHoverPreviewSlot::Pending(target);
                self.visual_presentation_target_count += 1;
                outcome.visual_presentation_target = Some(target);
            }
        } else {
...
mod tests {
    ...
    fn same_frame_hover_updates_coalesce_to_latest_target() {
        ...
        coalescer.record(TimelineHoverIntent::Target(hover_target(10)));
        coalescer.record(TimelineHoverIntent::Target(hover_target(40)));
        ...
        assert_eq!(outcome.visual_presentation_target, Some(hover_target(40)));
    }
    ...
    fn duplicate_target_does_not_republish_prepare_or_visual_intents() {
        ...
        TimelineHoverFrameCoalescer {
            latest_intent: Some(TimelineHoverIntent::Target(hover_target(25))),
        }
    ...
    fn visual_preview_disabled_still_emits_invisible_prepare_intent() {
        ...
        let outcome = TimelineHoverFrameCoalescer {
            latest_intent: Some(TimelineHoverIntent::Target(hover_target(60))),
        }
    ...
    fn leave_clears_visual_slot_and_active_prepare_target() {
        ...
        TimelineHoverFrameCoalescer {
            latest_intent: Some(TimelineHoverIntent::Target(hover_target(30))),
        }
        .finish(&mut state, true);
        let outcome = TimelineHoverFrameCoalescer {

//! Syntactic direct HDS `.f4m` classifier и startup adapter.

use super::*;

/// Stable root target и initial-only extractor fallback locator.
struct NativeHdsStartupAdapter {
    /// Reconstructible root без fragment/bootstrap material.
...
impl StartupUrlServiceAdapter for NativeHdsStartupAdapter {
    ...
    fn start(
        ...
        controller.start_native_hds_startup_job(
            self.source,
    ...
    fn into_media_open_source_request(
        ...
        Ok(crate::media_open::MediaOpenSourceRequest::Web(
            crate::media_open::WebMediaOpenRequest::native_hds(
                self.source,
                crate::media_open::NativeHdsOpenIntent::InitialWithYtDlpFallback {
...
/// Регистрирует только HTTP(S) path с case-insensitive `.f4m` suffix-ом.
pub(super) fn classify_native_hds_startup_url(argument: &str) -> ServiceClassifierResult {
    let Ok(parsed) = url::Url::parse(argument) else {
        return ServiceClassifierResult::NotUrl;
    };
    let has_f4m_suffix = parsed
        .path_segments()
        .and_then(|mut segments| segments.rfind(|segment| !segment.is_empty()))
        .is_some_and(|segment| segment.to_ascii_lowercase().ends_with(".f4m"));
    if !matches!(parsed.scheme(), "http" | "https") || !has_f4m_suffix {
        return ServiceClassifierResult::NotUrl;
    }
    ...
    ServiceClassifierResult::Supported(StartupUrlLocator::new(NativeHdsStartupAdapter {
        source: crate::media_open::NativeHdsUrl::new(target, safe_label),
        fallback_locator,
    }))
}
